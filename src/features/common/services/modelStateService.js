...
const { CENTRALIZED_API_KEYS, getApiKey } = require('../config/centralizedKeys.js');
...
class ModelStateService extends EventEmitter {
    ...
    async initialize() {
        ...
        await this._initializeCentralizedKeys();
    ...
    async _initializeCentralizedKeys() {
        console.log('[ModelStateService] Initializing centralized API keys...');
        try {
            for (const [provider, config] of Object.entries(CENTRALIZED_API_KEYS)) {
                if (config.api_key && config.api_key !== 'YOUR_ANTHROPIC_API_KEY_HERE') {
                    console.log(`[ModelStateService] Setting up centralized key for ${provider}`);
                    await this.setApiKey(provider, config.api_key);
                }
            }
            console.log('[ModelStateService] Centralized API keys initialized successfully');
        } catch (error) {
            console.error('[ModelStateService] Failed to initialize centralized keys:', error);
        }
    }
        ...
        for (const type of types) {
            ...
            if (!isCurrentModelValid) {
                ...
                if (availableModels.length > 0) {
                    // Prioritize Groq models for faster inference, then other API models
                    const groqModel = availableModels.find(model => {
                    ...
                    const newModel = groqModel || apiModel || availableModels[0];
    ...
    async setApiKey(provider, key) {
        ...
        // Check if we should use centralized keys
        const centralizedKey = getApiKey(provider);
        if (centralizedKey) {
            console.log(`[ModelStateService] Using centralized API key for ${provider}`);
            const finalKey = (provider === 'ollama' || provider === 'whisper') ? 'local' : centralizedKey;
