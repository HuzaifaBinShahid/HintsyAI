...
export class SettingsView extends LitElement {
    ...
    connectedCallback() {
        ...
        // Load usage stats after component is connected and window.api is available
        setTimeout(() => this.loadUsageStats(), 100);
    ...
    setupIpcListeners() {
        ...
        this._userStateListener = (event, userState) => {
            ...
            console.log('[SettingsView] Previous firebaseUser:', this.firebaseUser?.uid || 'null');
            
            if (userState && userState.isLoggedIn) {
                this.firebaseUser = userState;
                console.log('[SettingsView] User logged in:', userState.uid);
            } else {
                this.firebaseUser = null;
                console.log('[SettingsView] User logged out - clearing firebaseUser');
            }
            
            console.log('[SettingsView] New firebaseUser:', this.firebaseUser?.uid || 'null');
            
            // CRITICAL: Always trigger UI update when user state changes
            this.requestUpdate();
            console.log('[SettingsView] Requested UI update');
    ...
    handleFirebaseLogout() {
        console.log('[SettingsView] Firebase Logout clicked');
        console.log('[SettingsView] Current firebaseUser before logout:', this.firebaseUser?.uid || 'null');
        
        // Add visual feedback
        const wasLoggedIn = !!(this.firebaseUser && this.firebaseUser.isLoggedIn);
        console.log('[SettingsView] Was logged in:', wasLoggedIn);
        
        if (wasLoggedIn) {
            // Call the logout function
            window.api.settingsView.firebaseLogout().then(() => {
                console.log('[SettingsView] Logout API call completed');
                // Force a UI update after a short delay
                setTimeout(() => {
                    console.log('[SettingsView] Forcing UI update after logout');
                    this.requestUpdate();
                }, 1000);
            }).catch((error) => {
            ...
            console.log('[SettingsView] User was not logged in, no action needed');
        }
    }
    ...
    handleUpgradeClick() {
        ...
        const billingUrl = `${getWebUrl()}/settings/billing?t=${timestamp}`;
        
        console.log('[SettingsView] Opening billing URL:', billingUrl);
        
        // Open in external browser (system browser, not Electron)
        window.api.common.openExternal(billingUrl);
    }

    async loadUsageStats() {
        try {
            console.log('[SettingsView] Loading usage stats via IPC...');
            console.log('[SettingsView] window.api available:', !!window.api);
            console.log('[SettingsView] window.api.invoke available:', !!window.api?.invoke);
            
            if (!window.api || !window.api.invoke) {
                console.error('[SettingsView] window.api.invoke not available, retrying in 500ms...');
            ...
            if (this.firebaseUser && this.firebaseUser.uid) {
                ...
                console.log('[SettingsView] Using Firebase user ID for usage stats:', userId);
            } else {
                console.log('[SettingsView] No Firebase user available, will use current authenticated user from main process');
            }
            
            // Get usage stats via IPC from main process with proper parameters
            const params = userId ? { userId } : {};
            this.usageStats = await window.api.invoke('usage:getStats', params);
            console.log('[SettingsView] Usage stats received:', this.usageStats);
            this.requestUpdate();
        } catch (error) {
            console.error('[SettingsView] Error loading usage stats:', error);
            // Don't reset to free plan on error - keep existing stats if available
            if (!this.usageStats) {
                // Only set fallback if no stats exist at all
                this.usageStats = {
                    currentPlan: 'free',
                    planName: 'Free',
                    used: 0,
    ...
    async syncPlanFromWeb() {
        try {
            console.log('[SettingsView] Syncing plan from web...');
            
            if (!this.firebaseUser) {
                console.log('[SettingsView] No user logged in, cannot sync plan');
                return;
            }
            
            const result = await window.api.invoke('usage:syncPlanFromWeb', this.firebaseUser.uid);
            console.log('[SettingsView] Plan sync result:', result);
            
            if (result.success) {
                await this.loadUsageStats();
                console.log('[SettingsView] Plan synced successfully:', result.plan);
            } else {
                console.error('[SettingsView] Plan sync failed:', result.error);
            }
        } catch (error) {
            console.error('[SettingsView] Error syncing plan from web:', error);
        }
    }
    ...
    render() {
        ...
        const apiKeyManagementHTML = html`
            <div class="api-key-section">
                <div style="padding: 12px; background: rgba(0,122,255,0.1); border-radius: 8px; margin-bottom: 16px; border: 1px solid rgba(0,122,255,0.2);">
                                            <div style="font-weight: 500; margin-bottom: 4px; color: rgba(0,122,255,0.9);">🚀 AI Models Ready!</div>
                        <div style="font-size: 12px; color: rgba(255,255,255,0.8);">
                            All cloud AI models (GPT-4, Grok, Gemini) are pre-configured and ready to use. You can also set up local models like Ollama below.
                        </div>
                        <div style="display: flex; gap: 4px; margin-top: 8px;">
                            <button style="flex: 1; padding: 4px 8px; background: rgba(0,122,255,0.2); border: 1px solid rgba(0,122,255,0.3); border-radius: 4px; color: rgba(0,122,255,0.9); font-size: 11px; cursor: pointer;" @click=${() => this.loadUsageStats()}>
                                🔄 Refresh Usage
                            </button>
                            <button style="flex: 1; padding: 4px 8px; background: rgba(0,200,100,0.2); border: 1px solid rgba(0,200,100,0.3); border-radius: 4px; color: rgba(0,200,100,0.9); font-size: 11px; cursor: pointer;" @click=${this.syncPlanFromWeb}>
                                💎 Sync Plan
                            </button>
                        </div>
                </div>
                ...
                <!-- Usage Display -->
                <div style="padding: 12px; background: rgba(255,255,255,0.05); border-radius: 8px; margin-bottom: 16px; border: 1px solid rgba(255,255,255,0.1);">
                    <div style="font-weight: 500; margin-bottom: 8px; color: rgba(255,255,255,0.9);">📊 Usage This Month</div>
                    <div style="font-size: 12px; color: rgba(255,255,255,0.8); line-height: 1.4;">
                        <div style="margin-bottom: 4px;">
                            <strong>Plan:</strong> ${this.usageStats?.planName || 'Free'}
                        </div>
                        <div style="margin-bottom: 4px;">
                            <strong>Used:</strong> ${this.usageStats?.used || 0} / ${this.usageStats?.isUnlimited ? 'Unlimited' : (this.usageStats?.limit || 15)} requests
                        </div>
                        ${this.usageStats?.isUnlimited ? html`
                            <div style="color: rgba(0,255,0,0.8);">✓ Unlimited requests</div>
                        ` : html`
                            <div style="margin-bottom: 4px;">
                                <strong>Remaining:</strong> ${this.usageStats?.remaining ?? 15} requests
                            </div>
                            <div style="margin-bottom: 8px;">
                                <div style="background: rgba(255,255,255,0.1); border-radius: 4px; height: 6px; overflow: hidden;">
                                    <div style="background: linear-gradient(90deg, #007AFF, #0056CC); height: 100%; width: ${this.usageStats?.percentage || 0}%; transition: width 0.3s ease;"></div>
                                </div>
                            </div>
                        `}
                        ${this.usageStats?.currentPlan === 'free' && this.usageStats?.used >= 10 ? html`
                            <div style="padding: 8px; background: rgba(255,200,0,0.1); border-radius: 4px; margin-top: 8px; border: 1px solid rgba(255,200,0,0.2);">
                                <div style="font-size: 11px; color: rgba(255,200,0,0.9); margin-bottom: 4px;">⚠️ Approaching limit</div>
                                <button style="background: linear-gradient(135deg, #007AFF, #0056CC); color: white; border: none; border-radius: 4px; padding: 6px 12px; font-size: 11px; cursor: pointer;" @click=${this.handleUpgradeClick}>
                                    Upgrade to Pro
                                </button>
                            </div>
                        ` : ''}
                ...
                ${Object.entries(this.providerConfig)
                    ...
                    .map(([id, config]) => {
                        ...
                        // Hide centralized API key providers - they are configured automatically
                        // Commented out to hide from users since we're using centralized keys
                        /*
                        // Regular providers with centralized keys
                        const hasCentralizedKey = this.apiKeys[id] && this.apiKeys[id].length > 0;
                        return html`
                        <div class="provider-key-group">
                            <label for="key-input-${id}">${config.name} API Key</label>
                            ${hasCentralizedKey ? html`
                                <div style="padding: 8px; background: rgba(0,255,0,0.1); border-radius: 4px; font-size: 11px; color: rgba(0,255,0,0.8); margin-bottom: 8px;">
                                    ✅ ${config.name} is configured and ready to use
                                </div>
                                <div class="key-buttons">
                                   <button class="settings-button" @click=${() => this.handleSaveKey(id)} disabled>Using Centralized Key</button>
                                   <button class="settings-button danger" @click=${() => this.handleClearKey(id)}>Clear</button>
                                </div>
                            ` : html`
                                <input type="password" id="key-input-${id}"
                                    placeholder="Enter your own ${config.name} API Key (optional)" 
                                    .value=${this.apiKeys[id] || ''}
                                >
                                <div class="key-buttons">
                                   <button class="settings-button" @click=${() => this.handleSaveKey(id)}>Save Your Key</button>
                                   <button class="settings-button danger" @click=${() => this.handleClearKey(id)}>Clear</button>
                                </div>
                            `}
                        </div>
                        ...
                        */
                        ...
                        // Return null for centralized providers to hide them
                        return null;
                    }).filter(Boolean)}
                    
                    <!-- Available Models Info -->
                    <div style="padding: 12px; background: rgba(0,255,0,0.05); border-radius: 8px; margin-top: 16px; border: 1px solid rgba(0,255,0,0.1);">
                        <div style="font-weight: 500; margin-bottom: 8px; color: rgba(0,255,0,0.9);">📋 Available Models</div>
                        <div style="font-size: 12px; color: rgba(255,255,255,0.8); line-height: 1.4;">
                            <div style="margin-bottom: 4px;"><strong>Cloud Models (Ready to use):</strong> GPT-4, GPT-4o, Grok, Gemini</div>
                            <div style="margin-bottom: 4px;"><strong>Local Models (Setup required):</strong> Ollama (LLM), Whisper (STT)</div>
                            <div style="font-size: 11px; color: rgba(255,255,255,0.6);">Select your preferred models in the sections below.</div>
                        </div>
                    </div>
            </div>
        `;
        ...
        return html`
            <div class="settings-container">
                <div class="header-section">
                    <div>
                        <h1 class="app-title">Hintsy AI</h1>
                ...
                <div class="buttons-section">
                    ...
                    <div class="bottom-buttons">
                        ${(() => {
                            const isLoggedIn = this.firebaseUser && this.firebaseUser.isLoggedIn;
                            console.log('[SettingsView] Render - firebaseUser:', this.firebaseUser);
                            console.log('[SettingsView] Render - isLoggedIn:', isLoggedIn);
