...
class SttService {
    ...
    async flushMyCompletion() {
        ...
        if (!await firestoreUsageService.canMakeRequest()) {
            ...
            console.log('[SttService] Usage limit reached for transcription:', upgradeMessage);
            
            // Send usage limit message to renderer
            this.sendToRenderer('usage:limitReached', upgradeMessage);
            return;
        }
        ...
        console.log('[SttService] Usage recorded for successful transcription (Me)');
    ...
    async flushTheirCompletion() {
        ...
        // Record successful transcription usage
        await firestoreUsageService.recordRequest();
        console.log('[SttService] Usage recorded for successful transcription (Them)');
            ...
            if (this.modelInfo.provider === 'whisper') {
                ...
                if (message.serverContent?.turnComplete) {
                    if (this.myCompletionTimer) {
                        ...
                        this.flushMyCompletion().catch(console.error);
                    }
                    return;
                }
            ...
            if (this.modelInfo.provider === 'whisper') {
                ...
                if (message.serverContent?.turnComplete) {
                    if (this.theirCompletionTimer) {
                        ...
                        this.flushTheirCompletion().catch(console.error);
                    }
                    return;
                }
    ...
    async startMacOSAudioCapture() {
        ...
        // Check if SystemAudioDump binary exists
        if (!fs.existsSync(systemAudioPath)) {
            console.warn('SystemAudioDump binary not found at:', systemAudioPath);
            console.warn('System audio capture will be disabled. App will continue with microphone-only capture.');
            console.warn('To enable system audio capture, please ensure SystemAudioDump binary is available.');
            return false;
        }
        ...
        try {
            ...
            if (!this.systemAudioProc.pid) {
                console.error('Failed to start SystemAudioDump - no PID assigned');
                return false;
            }
        } catch (error) {
            console.error('Failed to spawn SystemAudioDump:', error.message);
            console.warn('System audio capture disabled. Continuing with microphone-only capture.');
            return false;
        }
