...
class ListenService {
    ...
    handleStartMacosAudio = this._createHandler(
        async () => {
            ...
            if (result === false) {
                console.warn('[ListenService] System audio capture failed, but app will continue with microphone-only capture');
                return { success: true, error: null, warning: 'System audio capture disabled - microphone only' };
            }
