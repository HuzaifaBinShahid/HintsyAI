...
module.exports = {
  ...
  initialize() {
    ...
    ipcMain.handle('get-web-url', () => process.env.Hintsy_AI_URL || 'http://localhost:3000');
    ...
    // Meeting Detection
    ipcMain.handle('meeting:start-monitoring', async () => {
        meetingDetectionService.startMonitoring();
        return { success: true };
    });
    
    ipcMain.handle('meeting:stop-monitoring', async () => {
        meetingDetectionService.stopMonitoring();
        return { success: true };
    });
    
    ipcMain.handle('meeting:get-current-info', async () => {
        return meetingDetectionService.getCurrentMeetingInfo();
    });
    
    ipcMain.handle('meeting:is-in-meeting', async () => {
        return meetingDetectionService.isCurrentlyInMeeting();
    });
    
    ipcMain.handle('meeting:test-detection', async () => {
        return await meetingDetectionService.testMeetingDetection();
    });
    ...
    // Usage Tracking
    const firestoreUsageService = require('../features/common/services/firestoreUsageService');
    ipcMain.handle('usage:getStats', async (event, params) => {
        const userId = params?.userId;
        console.log('[FeatureBridge] usage:getStats IPC handler called for user:', userId);
        
        if (userId) {
            // Set the current user to get their specific stats
            ...
            if (currentUser && currentUser.uid) {
                console.log('[FeatureBridge] No userId provided, using current authenticated user:', currentUser.uid);
                firestoreUsageService.setCurrentUser(currentUser.uid);
            } else {
                console.log('[FeatureBridge] No authenticated user found, using default');
            }
        }
        ...
        console.log('[FeatureBridge] Returning usage stats for user', userId || 'current', ':', response);
        return response;
    });
    ipcMain.handle('usage:canMakeRequest', async () => await firestoreUsageService.canMakeRequest());
    ipcMain.handle('usage:getUpgradeMessage', async () => await firestoreUsageService.getUpgradeMessage());
    ipcMain.handle('usage:setCurrentPlan', async (event, planName) => await firestoreUsageService.setCurrentPlan(planName));
    ipcMain.handle('usage:resetUsage', async () => await firestoreUsageService.resetUsage());
    ipcMain.handle('usage:upgradeToPro', async (event, userId) => {
        console.log('[FeatureBridge] usage:upgradeToPro called for user:', userId);
        // Set plan to pro and reset usage
        await firestoreUsageService.setCurrentPlan('pro');
        await firestoreUsageService.resetUsage();
        console.log('[FeatureBridge] User upgraded to Pro plan successfully');
        return { success: true };
    });

    ipcMain.handle('usage:syncPlanFromWeb', async (event, userId) => {
        console.log('[FeatureBridge] usage:syncPlanFromWeb called for user:', userId);
        try {
            // Use the auth service to sync plan
            const authService = require('../features/common/services/authService');
            await authService.syncPlanFromWeb(userId);
            
            return { success: true };
        } catch (error) {
            console.error('[FeatureBridge] Error syncing plan from web:', error);
