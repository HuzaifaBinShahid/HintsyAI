...
export const getSessions = async (): Promise<Session[]> => {
  if (isFirebaseMode()) {
    ...
    const firestoreSessions = await FirestoreSessionService.getAllSessions(uid);
...
export const getSessionDetails = async (sessionId: string): Promise<SessionDetails> => {
  if (isFirebaseMode()) {
    ...
    const [session, transcripts, aiMessages, summary] = await Promise.all([
      ...
      FirestoreAiMessageService.getMessages(uid, sessionId),
...
export const createPreset = async (data: { title: string, prompt: string }): Promise<{ id: string }> => {
  if (isFirebaseMode()) {
    throw new Error('Preset creation not implemented for Firebase mode');
  } else {
...
export const updatePreset = async (id: string, data: { title: string, prompt: string }): Promise<void> => {
  if (isFirebaseMode()) {
    throw new Error('Preset update not implemented for Firebase mode');
  } else {
...
export const deletePreset = async (id: string): Promise<void> => {
  if (isFirebaseMode()) {
    throw new Error('Preset deletion not implemented for Firebase mode');
  } else {
...
export const logout = async () => {
  try {
    // Always sign out from Firebase if user is authenticated
